...
This final optimization gave me initially 1.5 seconds total time for calculation runs between all vertices, and `it improved to 0.65s time` after I whitelisted in addition to running single sourced Dijkstra only from the list of allowed starting points to find out the necessary shortest trading route distances between space bases in a galaxy of a space simulator, and that is a satisfying end result for 1218 vertices, having 22896 edges in a directed graph.
